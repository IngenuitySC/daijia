    ...
    public List<NearByDriverVo> searchNearByDriver(SearchNearByDriverForm searchNearByDriverForm) {
        ...
        if(!CollectionUtils.isEmpty(content)) {
            ...
            while(iterator.hasNext()) {
                ...
                //当前订单里程距离（由 Tencent Map 根据起始点与终点预测出的里程距离）
                BigDecimal mileDistance = searchNearByDriverForm.getMileDistance();
                //司机可接受的订单里程距离 order_distance
                BigDecimal orderDistance = driverSet.getOrderDistance();
                //orderDistance==0 代表无订单里程距离限制
                //如果不等于0，则进行判断逻辑：可接受订单里程距离 - 当前订单里程距离  < 0,不符合条件
                if(orderDistance.doubleValue() != 0 && orderDistance.subtract(mileDistance).doubleValue() < 0) {
                    continue;
                }


                //当前订单接单距离(由Redis GEO 根据当前乘客位置与司机位置计算出的接单距离)
                BigDecimal currentDistance =
                        new BigDecimal(item.getDistance().getValue()).setScale(2, RoundingMode.HALF_UP);

                //司机可接受的接单距离 accept_distance
                BigDecimal acceptDistance = driverSet.getAcceptDistance();
                //acceptDistance==0 代表无接单距离限制
                //如果不等于0，则进行判断逻辑：可接受接单距离 - 当前订单接单距离  < 0,不符合条件
                if(acceptDistance.doubleValue() != 0 && acceptDistance.subtract(currentDistance).doubleValue() < 0) {
                    continue;
                }

                //将符合条件的司机NearByDriverVo加入list
